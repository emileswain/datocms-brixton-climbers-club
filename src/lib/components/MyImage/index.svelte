<script lang="ts">
    ...
    let {data, shredded = false, showCaption = false, width=460, height=259, ...rest}: Props = $props();
    let unmaskedData = $derived(readFragment(ResponsiveImageFragment, data));
    let caption = $derived(unmaskedData.title);
</script>

<figure class="figure" class:shredded={shredded} style:width={width+"px"} style:height={height+"px"}>
    <ResponsiveImage data={data} imgStyle="width:{width};height:{height}"/>
    {#if caption}
